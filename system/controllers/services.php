...
switch ($action) {
    ...
    case 'static':
        ...
        $ui->assign('xfooter', '<script type="text/javascript" src="ui/lib/c/static.js"></script>');
        ...
        $bws = ORM::for_table('tbl_plans')->distinct()->select("id_bw")->where('tbl_plans.type', 'Static')->findArray();
        $ids = array_column($bws, 'id_bw');
        if(count($ids)){
            $ui->assign('bws', ORM::for_table('tbl_bandwidth')->select("id")->select('name_bw')->where_id_in($ids)->findArray());
        }else{
    ...
    case 'static-add':
        ...
        $ui->assign('devices', $devices);
        run_hook('view_add_static'); #HOOK
        $ui->display('static-add.tpl');
        break;
    ...
    case 'static-edit':
        ...
        if ($d) {
            ...
            //select expired plan
            if ($d['is_radius']) {
                $exps = ORM::for_table('tbl_plans')->selects('id', 'name_plan')->where('type', 'Static')->where("is_radius", 1)->findArray();
            } else {
                $exps = ORM::for_table('tbl_plans')->selects('id', 'name_plan')->where('type', 'Static')->where("routers", $d['routers'])->findArray();
            }
            $ui->assign('exps', $exps);
            run_hook('view_edit_static'); #HOOK
            $ui->display('static-edit.tpl');
        } else {
