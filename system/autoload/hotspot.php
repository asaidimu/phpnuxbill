...
class HotspotRpc
{
  ...
  private function purchasePlan($params)
  {
    ...
    try {
      ...
      $push = new StkPush();
      list($response, $time) = $push->initiate($customer["phonenumber"], $plan["price"], $plan["plan_name"], "Hotspot");
  ...
  private function activatePlan($data)
  {
    ...
    $routerName = Router::getName($next_plan["routers"]);
    ...
    try {
      ...
      $router->connect_customer($customer, $customer["ip_address"], $customer["mac_address"], $routerName);
    } catch (\Exception $e) {
